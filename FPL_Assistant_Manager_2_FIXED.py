...
def squad_counts(squad: list[dict]) -> dict[int, int]:
    """Count players by club without allowing incomplete records to crash the app."""
    c = defaultdict(int)
    for p in squad or []:
        team_id = safe_int(p.get("team_id"), -1) if isinstance(p, dict) else -1
        if team_id >= 0:
            c[team_id] += 1
    return dict(c)
...
def legal_replacement(outgoing: dict, incoming: dict, squad: list[dict], bank: float) -> bool:
    # Be defensive here: current-squad records and model records can come from
    # different API/data sources. A missing club id should make a candidate
    # invalid, not crash the entire Streamlit app.
    if not outgoing or not incoming:
        return False
    if incoming.get("position") != outgoing.get("position"):
        return False
    if "price" not in incoming or "price" not in outgoing:
        return False
    if num(incoming.get("price")) > num(bank) + num(outgoing.get("price")) + 1e-9:
        return False
    if "id" not in incoming:
        return False
    ids = {safe_int(p.get("id"), -1) for p in squad}
    if safe_int(incoming.get("id"), -1) in ids:
        return False

    outgoing_team = outgoing.get("team_id")
    incoming_team = incoming.get("team_id")
    if outgoing_team is None or incoming_team is None:
        return False
    outgoing_team = safe_int(outgoing_team, -1)
    incoming_team = safe_int(incoming_team, -1)
    if outgoing_team < 0 or incoming_team < 0:
        return False

    counts = squad_counts(squad)
    counts[outgoing_team] = counts.get(outgoing_team, 0) - 1
    # The incoming club may not currently be represented in the squad.
    # Never index a normal dict with a brand-new key.
    counts[incoming_team] = counts.get(incoming_team, 0) + 1
    return counts[incoming_team] <= MAX_PER_CLUB


def transfer_suggestions(squad: list[dict], bank: float, free_transfers: int, elite_counts: dict[int, int]) -> list[dict]:
    owned = {safe_int(p.get("id"), -1) for p in squad if isinstance(p, dict) and p.get("id") is not None}
    suggestions = []
    for out in squad:
        candidates = [
            p for p in PLAYERS
...
def gemini_generate(
    ...
    """Generate Gemini output with retries and model fallbacks.

    Temporary 429/503/timeout-style failures are retried with exponential
    backoff. If a model remains unavailable, the next configured model is
    tried. Permanent authentication/request errors are not needlessly retried.
    """
    import time
    ...
    for model in GEMINI_MODELS:
        for attempt in range(max_retries):
            try:
                ...
                # Non-temporary errors (bad key, malformed request, etc.)
                # should move straight to the next configured model.
                if not temporary:
                    break

            # Retry temporary failures before abandoning this model.
            if attempt < max_retries - 1:
                time.sleep(2 ** attempt)  # 1s, 2s, 4s

    raise RuntimeError(
        "Gemini is temporarily unavailable. The app tried all configured "
        f"models with retries. Last error: {last_error}"
    )
...
"""
            try:
                ...
                st.error("⚠️ Creator AI is temporarily unavailable.")
                st.caption(
                    "The transcript was retrieved, but the AI analysis service "
                    "could not complete the request. Please try again shortly."
                )
...
"""
                with st.chat_message("assistant"):
                    try:
                        ...
                        st.error("⚠️ AI Assistant is temporarily unavailable.")
                        st.caption(
                            "The FPL data is still available. Please try your question again shortly."
                        )
                        with st.expander("Technical details"):
                            st.code(str(e))
